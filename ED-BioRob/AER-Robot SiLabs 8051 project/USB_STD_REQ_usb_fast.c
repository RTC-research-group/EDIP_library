...
extern BYTE USB_State;                          // Determines current usb device state
 extern idata unsigned char salidapc;
 //extern idata unsigned char entradapc;
...
{                                               // device configuration value
   ...
   else
   {
      ...
      {                                         // results in configuration being set to 1
         ...
         POLL_WRITE_BYTE(EINCSR2, 0xA0);  // Set DIRSEL to indicate endpoint 1 is IN
      //   entradapc=1;   //2 Free Slots
...
// direction = OUT
		POLL_WRITE_BYTE(EOUTCSR2, 0x80); // Double-buffering disabled
		 //END NEW
		 POLL_WRITE_BYTE(INDEX, 0);             // Set index back to endpoint 0
		salidapc=0;			//No data from PC available yet
      }
      else
      {
